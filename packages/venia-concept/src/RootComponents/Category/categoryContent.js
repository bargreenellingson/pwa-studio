...
class CategoryContent extends Component {
    ...
    componentDidMount() {
        ...
        sortOptions.forEach((option, index) => {
            if(option.value.sort === this.props.sort &&
    ...
    applySortFromURLParams = (paramsString) => {
        ...
        this.props.applySort(searchParams.get('sort'), searchParams.get('sortOrder'));
    }
    ...
    get sortComponent() {
        ...
        const className = isSortComponentOpen ? `${classes.sortOptions} ${classes.hidden}` :
            classes.sortOptions;

        return (
            (<div className={className}>
                <h2><span> Sort By {closeSortButton}</span></h2>
    ...
    get closeSortButton() {
        ...
        return (<Trigger action={() => toggleSortComponent()}> <Icon name="x"/> </Trigger>);
    }
